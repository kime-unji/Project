...
--게시판 글 번호에 사용할 시퀀스
create sequence web6_board_seq;

--댓글 테이블
create table web6_reply (
	...
	constraint reply_fk foreign key(boardnum)
		references web6_board(boardnum) on delete cascade
);

--댓글 번호에 사용할 시퀀스
create sequence web6_reply_seq;

--저장 예
insert into web6_member values 
	('aaa', 'aaa', '홍길동', '010-111-1111', '서울시', 'aaa@aaa.com');
	
insert into web6_board (boardnum, id, title, contents) 
	values (web6_board_seq.nextval, 'aaa', '글제목', '글내용');
	
insert into web6_reply (replynum, boardnum, id, text) 
	values (web6_reply_seq.nextval,  1 , 'aaa', '리플 내용');
